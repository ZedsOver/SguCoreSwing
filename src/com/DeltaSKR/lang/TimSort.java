...
class TimSort<T> {
     ...
     * method.
     *
     * Thanks to Stijn de Gouw, Jurriaan Rot, Frank S. de Boer, Richard Bubel
     * and Reiner Hahnle, this is fixed with respect to the analysis in "On the
     ...
     * closer hint is to the result, the faster this method will run. @param c
     * the comparator used to order the range, and to search @ret u rn the int
     * k, 0 <= k <= n such that a[b + k - 1] < key <= a[b + k], pretending that
     * a[b - 1] is minus infinity a nd a[b + n] is infinity. In other words, key
     * belongs at index b + k; or in other words, the first k elements of a
     * should precede key, and the last n - k should follow it.
     */
     ...
     * closer hint is to the result, the faster this method will run. @param c
     * the comparator used to order the range, and to search @ret u rn the int
     * k, 0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
     */
     ...
     * its twin, mergeHi should be called if len1 >= len2. (Either method may be
     * called if len1 == len2.)
     *
